...
class Donatur extends CI_Controller
{
	...
	function update_data_donatur($id)
	{
		$dateubah = date('Y-m-d H:i:s');
		$gambar = $this->M_data->get_data_by_id('tbl_donatur', array('id_donatur' => $id));
		foreach ($gambar->result() as $row) {
			$foto = $row->bukti_terima; //untuk mengambil gambar pada colom gambar di database berdasarkan ID
		}
		if ($_FILES["filefoto"]['error']) {
			...
			$this->M_data->update_data('tbl_donatur', $data, array('id_donatur' => $id));
			$this->session->set_flashdata('donatur', 'Diubah');
			redirect('admin/donatur');
		} else {
			unlink('./assets/bukti_penerima/' . $foto);
			$gambar = $this->upload('filefoto', 'bukti_penerima');
			$data = array(
				'nama_donatur' 		 => $this->input->post('nama_donatur'),
				'alamat_donatur' 	 => $this->input->post('alamat_donatur'),
				'nominal' 			 => $this->input->post('nominal'),
			...
			$this->M_data->update_data('tbl_donatur', $data, array('id_donatur' => $id));
			$this->session->set_flashdata('donatur', 'Diperbarui');
			redirect('admin/donatur');
		}
	}
}
