...
class Setting extends CI_Controller {
	...
	public function index()
	{
		...
		$data['conten'] = 'conten_koor/setting_user_koor';
		$data['title'] = 'Setting User koor';
	...
	public function tambah_user_koor(){
		...
		$this->M_data->simpan_data($table,$data);
		redirect('koor/setting/user_koor');
	}
	...
	public function kop_laporan()
	{
		$data['name'] 			= $this->session->userdata('nama');
		$data['lengkap'] 		= $this->session->userdata('lengkap');
		$data['conten'] = 'conten_koor/setting_kop';
	...
	function edit(){
		...
		redirect('koor/Setting/kop_laporan');
	}
	...
	public function master_kwitansi()
	{
		$data['name'] 			= $this->session->userdata('nama');
		$data['lengkap'] 		= $this->session->userdata('lengkap');
		$data['conten'] 	 = 'conten_koor/master_kwitansi';
		...
		if (!empty($_FILES['filefoto']['name'])) {
			...
			if($_FILES['filefoto']['name'])
			{
					if ($this->upload->do_upload('filefoto'))
					{
						...
						redirect('koor/setting/master_kwitansi');
					}else{
						//pesan yang muncul jika terdapat error dimasukkan pada session flashdata
						$this->session->set_flashdata("Gagal", "<center><b>Update Foto & Data Gagal</b></center>");
						redirect('koor/setting/master_kwitansi'); //jika gagal maka akan ditampilkan form upload
					}
			}
		}else {
			$data = array(
	...
	public function master_lokasi()
	{
		$data['name'] 			= $this->session->userdata('nama');
		$data['lengkap'] 		= $this->session->userdata('lengkap');
		$data['conten'] 	 	= 'conten_koor/setting_lokasi';
		...
		if (!empty($_FILES['filefoto']['name'])) {
			...
			if($_FILES['filefoto']['name'])
			{
					if ($this->upload->do_upload('filefoto'))
					{
						...
						redirect('koor/setting/master_lokasi');
					}else{
						//pesan yang muncul jika terdapat error dimasukkan pada session flashdata
						$this->session->set_flashdata("Gagal", "<center><b>Update Logo / Data Gagal</b></center>");
						redirect('koor/setting/master_lokasi'); //jika gagal maka akan ditampilkan form upload
					}
			}
		}else {
			$data = array(
	...
	public function ubah_sandi($id){
    	...
    	$data['conten']			= 'conten_koor/ubah_sandi';
		$data['hadir']			= $this->M_data->get_data_by_id('tbl_koordinator', array('id_koor' => $this->session->userdata('id_koor')));
    	$this->load->view('template_koor/conten',$data);
	}
	...
	public function update_sandi($id)
	{
		...
		$where = array('id_koor' => $this->session->userdata('id_koor'));
		$this->M_data->update_data($table,$data,$where);
		redirect('koor/Setting/ubah_sandi');
	}
}
